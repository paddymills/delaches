...
def deploy():
	...
	db.session.add_all([
		...
		Dues(type='Lifetime', amount=0.00),
		Dues(type='Junior', amount=10.00),
		Dues(type='Adult', amount=45.00),
		Dues(type='KeyFob', amount=5.00),
	])
	db.session.commit()
...
def import_data(filename):
	...
	data = data.with_columns(polars.col('type').replace_strict(["ADULT", "JUNIOR", "LIFETIME"], ["Adult", "Junior", "Lifetime"]))
	print(data)
...
if not args.deploy and not args.load:
	parser.print_help()